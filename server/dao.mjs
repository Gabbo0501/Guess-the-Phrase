...
export function updateGame(id, game) {
    return new Promise((resolve, reject) => {
        db.run(
            "UPDATE Games SET revealed = ?, vowelUsed = ?, usedLetters = ?, showFilm=?, gameCoins = ?, ended = ?, win = ? WHERE id = ?",
            [game.revealed, game.vowelUsed, game.usedLetters, game.showFilm, game.gameCoins, game.ended, game.win, id], function(err) {
                if (err) return reject(err);
                resolve();
            }
        );
